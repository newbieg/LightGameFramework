...
// might be over my head with this class atm.
...
textBox::textBox()
{
	...
	lines[0]->setPos(boundingBox.x, boundingBox.y);
	lines[0]->fullRender();
...
void textBox::handleEvent(SDL_Event & ev)
{
	if(ev.type == SDL_KEYDOWN)
	{
		...
		else if(cursor < lines[currentLine]->getTextPtr()->length())
		{
...
void textBox::draw(SDL_Surface * dest)
{
	if(shouldShowBkg)
	{
		background.setColor(bkgColor);
	}
	for(int i = yoffset; i < lines.size() && i < boundingBox.h /lines[0]->getFontSize(); i ++)
	{	
		lines[i]->quickRender();
		if(!lines[i]->getTextPtr()->empty())
		{
			lines[i]->draw(background.getImage());
		}
	}
...
void textBox::newLine()
{
			...
			tmp->setFontSize(lines[0]->getFontSize());
			tmp->fullRender();
			tmp->setPos(boundingBox.x, lines[currentLine]->getPos().y + tmp->getFontSize());
