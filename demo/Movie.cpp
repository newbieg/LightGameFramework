/* A testing ground for creating a recorded game.
 * Not a recomendation for game use, 
 * I have no experience with libpng,
 * it slows down performance beyond playability
 * and that's without error checking.
 * right now it's only good for the occasional
 *  snapshot of current events which may be useful
 *  as a backflash intro
 */
...
void numStr::increment()
{
	text[text.length() - 1] ++;
	int res = text.length() - 1;
	for(int i = 0; i < res; i ++)
	{
		while(text[res - i] > '9' && res - i - 1 >= 0)
		{
			text[res - i] -= 10;
			text[res - i - 1] ++;
		}
...
//		man.draw(screen);
		...
		string fileName = "movs/blah";
		bob.increment();
		fileName += bob.text;
		fileName += ".png";
		vector <char> fn(fileName.c_str(), fileName.c_str() + fileName.size() + 1);
		writeImage(screen, &fn[0]); // nasty workaround to using string
		limit.fc ++;
