...
// ====================== Info Bar Class
class inf: public item
{
	public:
	inf();
	void checkClick(SDL_Event * e); // Check if a button was clicked
		
	private:
	txt teamstate, sel_hp, gameState;
	unsigned int color; // bkg color changes to match player turn
...
class colorKit
{
	public:
		colorKit();
		colorKit(SDL_Surface* screen, int r, int g, int b);
		void setBaseColor(int r, int g, int b);
		unsigned int getcolor();
		unsigned int getfg();
		unsigned int getbg();
		unsigned int getboarder();
		unsigned int getup();
		unsigned int getdown();
		unsigned int gethover();
...
colorKit::colorKit(SDL_Surface * screen, int r, int g, int b)
{
	color.push_back(SDL_MapRGB(screen->format, r, g, b));
	color.push_back(distWhite(screen, r, g, b, 4));
	color.push_back(distWhite(screen, r, g, b, 10));
	color.push_back(distWhite(screen, r, g, b, 2));
	color.push_back(distWhite(screen, r, g, b, 3));
	color.push_back(distWhite(screen, r, g, b, 20));
	color.push_back(distWhite(screen, r, g, b, 7));
}
...
//			cout << endl;
		...
		// soldiers capturing an enemy county
